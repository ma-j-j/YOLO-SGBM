...
public:
  ...
  ~Yolo() {}
  bool readModel(cv::dnn::Net &net, std::string &netPath, bool isCuda);                                                                                              // 读模型
  bool Detect(cv::Mat &SrcImg, cv::dnn::Net &net, std::vector<Output> &output);                                                                                      // 检测
  void drawPred(cv::Mat &img, std::vector<Output> result, std::vector<cv::Scalar> color);                                                                            // 画框
  void matching(cv::Mat &limg, cv::Mat &rimg, std::vector<Output> Lresultinput, std::vector<Output> Rresultinput, std::vector<std::vector<int>> &OutputInformation); // 模板匹配

  // 筛选标签
  std::map<int, std::string> class_select = {{0, "person"}};
